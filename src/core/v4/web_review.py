...
<script>
const TOKEN=__TOKEN__;const DRAFT_KEY=`parallel_v4_vote_drafts_${TOKEN}`;let currentBlock=null;let revealed=false;let voteDrafts={};try{voteDrafts=JSON.parse(localStorage.getItem(DRAFT_KEY)||'{}')}catch(_){voteDrafts={}}
...
async function submitVote(choice){const block=currentBlock;const note=document.querySelector('#voteNote').value;await api('/api/comparison-votes',{method:'POST',body:JSON.stringify({block,choice,blinded:!revealed,note})});delete voteDrafts[block];localStorage.setItem(DRAFT_KEY,JSON.stringify(voteDrafts));await loadBlock(block,!revealed);const s=await api('/api/state');renderBlocks(s.blocks)}

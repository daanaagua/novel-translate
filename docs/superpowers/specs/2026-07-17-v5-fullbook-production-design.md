...
`Little, Big` 冷启动揭示了三项不能留到全量运行再处理的问题，因此生产协议升级为 `v5-book-3`：

- 原导入器把 `Book One` 至 `Book Six` 全部放入同一卷，重复的罗马数字章节 ID 会覆盖前卷。导入器现在识别英文数字卷名，并对任何重复章节 ID 加稳定序号；完整原文为 33 章、231 blocks，而非被覆盖后的 6 章、43 blocks。
- 固定的“问题侦察—检索—定案—翻译”使 73 source-token 的书名页耗费 8 次模型调用和约 124 秒。全书主路径现改为翻译代理按需调用 `request_translation_evidence`：问题必须附带当前窗口原文中真实出现的 1–3 个英文词形，Kernel 只返回最多 6 条、3,600 字符、满足位置通道的本地证据；没有具体措辞疑问时直接翻译。
- 默认窗口从 7,000 source-token/6 blocks 收紧到 2,600 source-token/3 blocks，降低长输出被供应商终止的概率。模型供应商错误使用独立错误类型，整书立即中止且窗口回到 pending，不再消耗内部重试或伪装为人工翻译任务。

翻译提交可同时携带最多四条可选叙事记忆候选。Kernel 只持久化置信度至少 0.9、subject form 确实出现在当前窗口、能映射到稳定实体、并能绑定当前块 evidence ID 的事实；从下一窗口起按人物命中与位置边界投影。该机制不增加独立摘要调用。

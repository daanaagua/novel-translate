...
上下文构建必须返回准确依赖 ID 和各部分字符数。任何必需部分不能因总预算被静默截断。旧译文基线永远是措辞和查漏参考，英文原文是唯一事实依据。
...
`memory_summary` 只作为可读审计摘要，不再承担唯一的跨块状态。补充记忆候选必须引用当前英文原文证据，默认置信度低于预映射候选；如果只是中文措辞推断而无法在英文原文定位，则拒绝入库。

### 11.2 润色

润色模型接收英文原文、初译、语义义务、结构约束、有界叙事上下文、样式状态和可选旧译文。它只能返回最终译文和质量警告，不能修改词库、记忆、关系或话语状态。
